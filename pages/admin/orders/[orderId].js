...
import { cancelOrders, fetchOrderDetails } from 'redux/actions/orderActions'
import { calculateAmountOfOrder, checkIfEmpty, getObjectValue } from 'utils/helpers'
 ...
 * @returns
 */
function OrderDetails({ fetchOrderDetails, order, cancelOrders }) {
      ...
      <div className={classes.bgOrderDetail}>
        <div className={classes.orderDetail_Head}>
          <div style={{ flexGrow: 1 }}>
            ...
            <BreadCrumb
              routes={[{ name: 'Order list', link: '/admin/orders' }, { name: 'Order details' }]}
            />
            {!checkIfEmpty(order?.notes) && (
          ...
          <div className={classes.detail_Action}>
            {(order?.status === 6 || order?.status === 1 || order?.status === 2) && (
              <Button
                type='submit'
                variant='outlined'
            ...
            <>
              <div className={classes.orderDetail_Root}>
                ...
                <Grid container spacing={3} direction='row' className={classes.orderGrid_Row}>
                  ...
                  {(order?.shippingAddress?.country === 'EU' ||
                    order?.shippingAddress?.country === 'UK') && (
                    <Grid item xs={12} sm={6} md={4} lg={3} xl={3} className={classes.orderGrid}>
                      <Typography variant='body1'> IOSS</Typography>
                      <Typography variant='body2'>{order?.IOSS || '---'}</Typography>
                    </Grid>
                  )}
                </Grid>
              </div>
